...
namespace contractverify
{
    ...
    SuccessTestInfo successContractTestInfos[] =
    {
        {
    ...
    FailureTestInfo failureContractTestInfos[] =
    {
        {
    ...
    INSTANTIATE_TEST_SUITE_P(CVST,
        ...
        testing::ValuesIn(successContractTestInfos),
    ...
    INSTANTIATE_TEST_SUITE_P(CVFT,
        ...
        testing::ValuesIn(failureContractTestInfos),
    ...
    //-------------------------------------------------------------------------
    // Oracle interfaces tests (reusing most test cases of contracts)

    class OracleInterfaceVerifySuccessTest : public testing::TestWithParam<SuccessTestInfo> {};

    class OracleInterfaceVerifyFailureTest : public ContractVerifyFailureTest {};

    TEST_P(OracleInterfaceVerifySuccessTest, PassesComplianceCheck)
    {
        const SuccessTestInfo& info = GetParam();
        std::filesystem::path filepath = std::filesystem::path(testfiles::baseDir).append(info.filename);
        std::unique_ptr<cppast::CppCompound> ast = contractverify::parseAST(filepath.string());
        ...
        std::string structName = contractverify::findOracleInterfaceStructNameAndCheckTopLevel(*ast);
        EXPECT_NE(structName, "");

        EXPECT_TRUE(contractverify::checkCompliance(*ast, structName, FileType::ORACLE_INTERFACE));
    }

    TEST_P(OracleInterfaceVerifyFailureTest, FailsWithExpectedError)
    {
        const FailureTestInfo& info = GetParam();
        std::filesystem::path filepath = std::filesystem::path(testfiles::baseDir).append(info.filename);
        std::unique_ptr<cppast::CppCompound> ast = contractverify::parseAST(filepath.string());
        ...
        std::string structName = contractverify::findOracleInterfaceStructNameAndCheckTopLevel(*ast);
        if (structName != "")
            EXPECT_FALSE(contractverify::checkCompliance(*ast, structName, FileType::ORACLE_INTERFACE));
        EXPECT_THAT(customBuffer.str(), testing::StrEq(info.expectedErrorMessage));
    }

    SuccessTestInfo successOracleInterfaceTestInfos[] =
    {
        {
            "test_ok_function_call.h",
            "FunctionCall"
        ...
        {
            "oracle_interfaces/test_ok_DogeShareValidation.h",
            "OracleInterfaceDogeShareValidation"
        },
        {
    ...
    FailureTestInfo failureOracleInterfaceTestInfos[] =
    {
        ...
        {
            "test_fail_global_constant.h",
            "[ ERROR ] The oracle interface must contain exactly one struct definition. Other definition/declarations are forbidden.\n",
            "GlobalConstant"
        },
        {
            "test_fail_global_variable.h",
        ...
        {
            "test_fail_input_output_1.h",
            "[ ERROR ] NotAllowed is not allowed as input/output type. The input and output structs of contract user procedures and functions may only use integer and boolean types (such as uint64, sint8, bit) as well as id, Array, and BitArray, and struct types containing only allowed types.\n",
            "InputOutputTypes1"
        },
        {
            "test_fail_input_output_2.h",
            "[ ERROR ] Helper is not allowed as input/output type. The input and output structs of contract user procedures and functions may only use integer and boolean types (such as uint64, sint8, bit) as well as id, Array, and BitArray, and struct types containing only allowed types.\n",
            "InputOutputTypes2"
        },
        {
            "test_fail_input_output_3.h",
        ...
        {
            "test_fail_input_output_9.h",
            "[ ERROR ] num8BitHashSet is not allowed as input/output type. The input and output structs of contract user procedures and functions may only use integer and boolean types (such as uint64, sint8, bit) as well as id, Array, and BitArray, and struct types containing only allowed types.\n",
            "InputOutputTypes9"
        },
        {
            "test_fail_input_output_10.h",
        ...
        {
            "test_fail_input_output_11.h",
            "[ ERROR ] unnamed struct is not allowed as input/output type. The input and output structs of contract user procedures and functions may only use integer and boolean types (such as uint64, sint8, bit) as well as id, Array, and BitArray, and struct types containing only allowed types.\n",
            "InputOutputTypes11"
        },
        {
            "test_fail_input_output_12.h",
    ...
    INSTANTIATE_TEST_SUITE_P(OIVST,
        OracleInterfaceVerifySuccessTest,
        testing::ValuesIn(successOracleInterfaceTestInfos),
        [](const testing::TestParamInfo<OracleInterfaceVerifySuccessTest::ParamType>& info)
        { return info.param.testName; }
    );

    INSTANTIATE_TEST_SUITE_P(OIVFT,
        OracleInterfaceVerifyFailureTest,
        testing::ValuesIn(failureOracleInterfaceTestInfos),
        [](const testing::TestParamInfo<OracleInterfaceVerifyFailureTest::ParamType>& info)
        { return info.param.testName; }
    );
